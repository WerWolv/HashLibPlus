...
// Global Functions
HashLibByteArray GenerateByteArrayInRange(const byte start, const UInt32 count) {
	HashLibByteArray result = HashLibByteArray();
	...
	return result;
} // end function GenerateByteArrayInRange

HashLibByteArray GenerateRepeatingBytes(const byte start, const UInt32 count) {
	HashLibByteArray result = HashLibByteArray();
...
IHash NullHashInstance = nullptr;

// "Hash"
const HashLibByteArray HMACShortKeyBytes = { 0x48, 0x61, 0x73, 0x68 };

// "This is a very long key used to test HMAC functionality when key length is greater than the hash blocksize"
const HashLibByteArray HMACLongKeyBytes = {
...
const HashLibByteArray EmptyBytes = {};

// "HashLib4Pascal"
const HashLibByteArray DefaultDataBytes = { 0x48, 0x61, 0x73, 0x68, 0x4C, 0x69, 0x62, 0x34, 0x50, 0x61, 0x73, 0x63, 0x61, 0x6C };

const HashLibByteArray SmallLettersAToEBytes = GenerateByteArrayInRange(0x61, 5);

//123456789
const HashLibByteArray OneToNineBytes = Converters::ConvertStringToBytes(OneToNine);

const HashLibByteArray ZeroToThreeBytes = GenerateByteArrayInRange(0x0, 4);

const HashLibByteArray ZeroToOneHundredAndNinetyNineBytes = GenerateByteArrayInRange(0x0, 200);

const HashLibByteArray ZeroToThirtyOneBytes = GenerateByteArrayInRange(0x00, 32);
const HashLibByteArray ZeroToSixtyThreeBytes = GenerateByteArrayInRange(0x00, 64);
const HashLibByteArray MaxUInt32Bytes = GenerateRepeatingBytes(0xFF, sizeof(UInt32));
const HashLibByteArray MaxUInt64Bytes = GenerateRepeatingBytes(0xFF, sizeof(UInt64));
const HashLibByteArray ZeroToFifteenBytes = GenerateByteArrayInRange(0x0, 16);
const HashLibByteArray ZeroToSevenBytes = GenerateByteArrayInRange(0x00, 8);

// "HashLib4Pascal012345678HashLib4Pascal012345678HashLib4Pascal012345678HashLib4Pascal012345678"
const HashLibByteArray ChunkedDataBytes = {
...
const HashLibByteArray ASCIICharacterBytes = GenerateByteArrayInRange(0x40, 32);

// "My Tagged Application"
const HashLibByteArray CustomizationMessageBytes = {
	0x4D, 0x79, 0x20, 0x54, 0x61, 0x67, 0x67, 0x65, 0x64, 0x20, 0x41, 0x70, 0x70, 0x6C, 0x69, 0x63, 0x61, 0x74,
	0x69, 0x6F, 0x6E
};

// Hash Clone TestHelper Variables
const HashLibByteArray MainData = Converters::ConvertStringToBytes(DefaultData);

const HashLibByteArray::const_iterator start = MainData.begin();
const HashLibByteArray::const_iterator mid = MainData.begin() + MainData.size() - 3;
const HashLibByteArray::const_iterator last = MainData.end();
